...
static void ClipOffset(Sint16 *x, Sint16 *y)
{
	 ...
	 */
	/* Modified for GP2X Wiz by ikari 2010.1.30 ===================================== */
	...
	{
		if ( SDL_VideoSurface->offset ) {
			*x -= ((SDL_VideoSurface->offset/SDL_VideoSurface->pitch)-video->width_odd);
			*y -= (SDL_VideoSurface->offset%SDL_VideoSurface->pitch)/
					SDL_VideoSurface->format->BytesPerPixel;
		}
	}
...
int SDL_PrivateMouseMotion(Uint8 buttonstate, int relative, Sint16 x, Sint16 y)
{
	...
	Sint16 Yrel;
#if SDL_VIDEO_DRIVER_GP2XWIZ
	SDL_VideoDevice* video=current_video; /* modified by ikari 2010.2.15 */
#endif
	...
	else
	{
		...
		{
			if ( x >= SDL_VideoSurface->h )
				X = SDL_VideoSurface->h-1;
			else
				X = (Uint16)x;
		}
#else
	...
	else
	{
		...
		{
			if ( y >= SDL_VideoSurface->w )
				Y = SDL_VideoSurface->w-1;
			else
				Y = (Uint16)y;
		}
#else
