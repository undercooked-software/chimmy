...
#if SDL_VIDEO_DRIVER_GP2XWIZ  /* modified by ikari 2010.5.28 */
...
int SDL_DisplayYUVOverlay(SDL_Overlay *overlay, SDL_Rect *dstrect)
{
...
#if SDL_VIDEO_DRIVER_GP2XWIZ  /* modified by ikari 2010.4.30 */
	if(!current_video->rotate_screen)
	{
	...
	{
		if ( (dstx+dstw) > current_video->screen->h ) {
			int extra = (dstx+dstw - current_video->screen->h);
			srcw -= (extra * overlay->w) / dstrect->w;
			dstw -= extra;
		}
	}
	...
	{
		if ( (dsty+dsth) > current_video->screen->w ) {
			int extra = (dsty+dsth - current_video->screen->w);
			srch -= (extra * overlay->h) / dstrect->h;
			dsth -= extra;
		}
	}
