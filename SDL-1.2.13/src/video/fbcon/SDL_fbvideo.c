...
/* modified by ikari 2010.3.3 */
static int mem_fd=-1;
static volatile unsigned int* memregs32=NULL;
				...
				int width, int height, int bpp, Uint32 flags)
{
	...
	if(bpp==8)
	{
		current->flags |= SDL_HWPALETTE;
		for(i=0;i<256;i++)
			memregs32[0x4070>>2]=i<<24;
		memregs32[0x4058>>2]|=0x10;
	}
