...
/* Modified for GP2X Wiz by ikari : 2010.1.23 */
static unsigned int is_shadow=0; 
...
int SDL_ShowCursor (int toggle)
{
	...
	if(((SDL_VideoSurface->flags&SDL_OPENGL)==SDL_OPENGL)||
		((SDL_VideoSurface->flags&SDL_OPENGLBLIT)==SDL_OPENGLBLIT))
	{
		toggle=SDL_DISABLE;
	}
#endif
...
void SDL_MouseRect(SDL_Rect *area)
{
	...
	*area = SDL_cursor->area;
	...
	{
		clip_diff = (area->x+area->w)-SDL_VideoSurface->h;
		if ( clip_diff > 0 ) {
			area->w = area->w < clip_diff ? 0 : area->w-clip_diff;
		}
		clip_diff = (area->y+area->h)-SDL_VideoSurface->w;
		if ( clip_diff > 0 ) {
			area->h = area->h < clip_diff ? 0 : area->h-clip_diff;
		}
	}
...
static void SDL_DrawCursorFast(SDL_Surface *screen, SDL_Rect *area)
{
	...
	/* Modified for GP2X Wiz by ikari 2010.1.23 ===================================== */
...
#if SDL_VIDEO_DRIVER_GP2XWIZ
		if((screen!=SDL_VideoSurface)||(!video->rotate_screen))
		{
		...
		else /* GP2X Wiz - Rotate cursor on 320x240mode */
		{
			dst = (Uint8 *)screen->pixels +
                       		(screen->h-SDL_cursor->area.x-1)*screen->pitch +
                       		SDL_cursor->area.y+area->y;
			dstskip = screen->pitch;
		...
		{
			dst = (Uint16 *)screen->pixels +
                       		(screen->h-SDL_cursor->area.x-1)*(screen->pitch>>1) +
                       		SDL_cursor->area.y+area->y;
			dstskip = screen->pitch>>1;
	...
	/* =============================================================================== */
	    ...
	    case 4: {
#if SDL_VIDEO_DRIVER_GP2XWIZ /* 2010.5.23 ikari */
		if((screen!=SDL_VideoSurface)||(!video->rotate_screen))
		{
			Uint32 *dst;
			int dstskip;
		...
		else /* GP2X Wiz - Rotate cursor on 320x240mode */
		{
			...
			dst = (Uint32 *)screen->pixels +
                       		(screen->h-SDL_cursor->area.x-1)*(screen->pitch>>2) +
                       		SDL_cursor->area.y+area->y;
			dstskip = screen->pitch>>2;
...
static void SDL_DrawCursorSlow(SDL_Surface *screen, SDL_Rect *area)
{
	...
	else
	{
		dst = (Uint8 *)screen->pixels +
                       		(screen->h-SDL_cursor->area.x-1)*screen->pitch +
                       		(SDL_cursor->area.y+area->y)*dstbpp;
		dstskip = screen->pitch;
	}
#else
	dst = (Uint8 *)screen->pixels +
		...
		else /* GP2X Wiz - Rotate cursor on 320x240mode */
		{
			for ( h=area->h; h; h-- ) {
				...
				for ( x=0; x<SDL_cursor->area.w; ++x ) {
					...
					if ( (x >= minx) && (x < maxx) ) {
						if ( maskb & 0x80 ) {
							SDL_memset(d, pixels8[datab>>7], dstbpp);
						}
					}
					maskb <<= 1;
					datab <<= 1;
		...
		else /* GP2X Wiz - Rotate cursor on 320x240mode */
		{
			for ( h=area->h; h; h-- ) {
				...
				for ( x=0; x<SDL_cursor->area.w; ++x ) {
					...
					if ( (x >= minx) && (x < maxx) ) {
						if ( maskb & 0x80 ) {
							SDL_memset(d, pixels[datab>>7], dstbpp);
						}
					}
					maskb <<= 1;
					datab <<= 1;
...
void SDL_DrawCursorNoLock(SDL_Surface *screen)
{
	...
	{ int w, h, screenbpp;
	  ...
	  {
		is_shadow=0;
	  	src = (Uint8 *)screen->pixels + (screen->h-area.x-1) * screen->pitch +
                                          	area.y * screenbpp;
...
void SDL_EraseCursorNoLock(SDL_Surface *screen)
{
	...
	{ int w, h, screenbpp;
	  Uint8 *src, *dst;
	  Uint32 dstskip=screen->pitch;
	  Uint8 *src2; /* modified by ikari 2010.3.8 */
	  ...
	  src2=src; /* modified by ikari 2010.3.8 */

	  /* Modified for GP2X Wiz by ikari 2010.1.23 ===================================== */
#if SDL_VIDEO_DRIVER_GP2XWIZ
	  if((screen!=SDL_VideoSurface)||(!video->rotate_screen))
	  {
	  	dst = (Uint8 *)screen->pixels + area.y * dstskip +
                                          	area.x * screenbpp;
		...
		if(video->rotate_screen)
		{
			...
			dstskip=rotation_buffer->pitch;
			dst = (Uint8*)rotation_buffer->pixels + (screen->w-area.x-1)*dstskip +
								area.y*screenbpp + SDL_VideoSurface->offset;
			...
			else
			{
				if(screenbpp==2)
				{
					Uint16* srcpixel=(Uint16*)src2;
					Uint16* dstpixel=(Uint16*)dst;
				...
				{
					Uint32* srcpixel=(Uint32*)src2;
					Uint32* dstpixel=(Uint32*)dst;
	  ...
	  else /* GP2X Wiz - Rotate cursor on 320x240mode */
	  {
		if(is_shadow)
		{
	  		dst = (Uint8 *)screen->pixels + (screen->h-area.x-1) * dstskip +
                                          	area.y * screenbpp;
			...
			if(video->rotate_screen)
			{
				SDL_Surface* rotation_buffer=video->rotation_buffer;
				dstskip=rotation_buffer->pitch;
				dst = (Uint8*)rotation_buffer->pixels + (screen->h-area.x-1)*dstskip +
									area.y*screenbpp + screen->offset;
