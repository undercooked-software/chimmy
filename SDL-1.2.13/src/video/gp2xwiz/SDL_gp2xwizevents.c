 ...
 */
static void GP2XWIZ_vgainitkeymaps(int fd)
{
...
int GP2XWIZ_InGraphicsMode(_THIS)
{
	return((keyboard_fd >= 0) && (saved_kbd_mode >= 0));
}

int GP2XWIZ_EnterGraphicsMode(_THIS)
{
	struct termios keyboard_termios;

	/* Set medium-raw keyboard mode */
	if ( (keyboard_fd >= 0) && !GP2XWIZ_InGraphicsMode(this) ) {
...
void GP2XWIZ_LeaveGraphicsMode(_THIS)
{
	if ( GP2XWIZ_InGraphicsMode(this) ) {
...
void GP2XWIZ_CloseKeyboard(_THIS)
{
	if ( (keyboard_fd >= 0 ) 
		|| (vt_fd >= 0)	/* For GP2X */
	)
	{
		GP2XWIZ_LeaveGraphicsMode(this);
		if ( keyboard_fd > 0 ) {
			close(keyboard_fd);
		}
...
int GP2XWIZ_OpenKeyboard(_THIS)
{
	/* Open only if not already opened */
 	if ( keyboard_fd < 0 ) {
		...
		/* Set up keymap */
		GP2XWIZ_vgainitkeymaps(keyboard_fd);
#endif
 	}
 	return(keyboard_fd);
}
...
static enum {
	...
	MOUSE_MS,
	MOUSE_BM,
	//MOUSE_ELO, -Modified by ikari 2010.1.23
	MOUSE_TSLIB,
	NUM_MOUSE_DRVS
} mouse_drv = MOUSE_NONE;

void GP2XWIZ_CloseMouse(_THIS)
{
#if SDL_INPUT_TSLIB
	if (ts_dev != NULL) {
		ts_close(ts_dev);
...
#if SDL_INPUT_TSLIB
void GP2XWIZ_vgamousecallback(int button, int relative, int dx, int dy);

static void (*wiz_ts_callback)(int,int,int);

static void wiz_ts_callback_320x240(int button,int dx,int dy)
{
	GP2XWIZ_vgamousecallback(button, 0, dx, dy);
}

static void wiz_ts_callback_240x320(int button,int dx,int dy)
{
	GP2XWIZ_vgamousecallback(button, 0, dy, 319-dx);
}
#endif
/* ============================================================ */

int GP2XWIZ_OpenMouse(_THIS)
{
	int i;
	const char *mousedev;
	const char *mousedrv;
...
#if 0 /* Modified by ikari 2010.1.23 */
...
static void handle_tslib(_THIS)
{
	...
	if(!init)
	{
		if((SDL_VideoSurface->flags&WIZ_240X320)==WIZ_240X320)
			wiz_ts_callback=wiz_ts_callback_240x320;
		else
			wiz_ts_callback=wiz_ts_callback_320x240;
	...
	while (ts_read(ts_dev, &sample, 1) > 0) {
		...
		(*wiz_ts_callback)(button,sample.x,sample.y);
	}
	return;
}
#endif /* SDL_INPUT_TSLIB */
...
static void handle_mouse(_THIS)
{
	...
	for ( i=0; i<(nread-(packetsize-1)); i += packetsize ) {
			...
			case MOUSE_IMPS2:
				...
				switch (mousebuf[i+3]&0x0F) {
				    ...
				    case 0x0F: /* DY = +1 (map button 4) */
                                       GP2XWIZ_vgamousecallback(button | (1<<3),
                                                           1, 0, 0);
					break;
				    case 0x01: /* DY = -1 (map button 5) */
                                       GP2XWIZ_vgamousecallback(button | (1<<4),
                                                           1, 0, 0);
					break;
				}
				break;
		...
		GP2XWIZ_vgamousecallback(button, relative, dx, dy);
	}
	if ( i < nread ) {
...
#if 0 /* modified by ikari 2010.1.30 */
...
/* ghcstop 041206: new keymapping ......^^, only ascii code만 먹게 바꿈....
   왜냐하면 serial console일 경우 키가 ascii만 먹게 되어 있넹....^^ 머 어쩔 수 없지롱
   */

/* SDL_types.h
#define SDL_TABLESIZE(table)	(sizeof(table)/sizeof(table[0]))

SDLKey // SDL_keysym.h에 enum형으로 선언되어 있음
static SDLKey keymap[128];

// keymap은 SDL_keysym.h에 있는 ASCII code range안에서만 설정한다.
대문자들은 뺀다(왜냐하면 SDL의 경우 shift눌린거랑 아닌거랑 차이를 두기 때문인것 같당....)
==> 하여간 shift눌러서 찍는 넘들은 다 뺀다..;
*/

void GP2XWIZ_InitOSKeymap(_THIS)
{
	int i;
	...
	for (i=0; i<SDL_TABLESIZE(vt_keymap); ++i)  // 128개 까정
		vt_keymap[i] = 0; // all key map clear ==> 몽땅 SDLK_UNKNOWN로 일단 세팅

	vt_keymap[0] =  SDLK_UNKNOWN	;
	vt_keymap[3] =  SDLK_CRTL_C	;// ghcstop 041206 add: vt_keymap을 뜯어고치면서 ctrl+c도 key event로 먹게 만든다.
...
static SDL_keysym *TranslateVTKey(int asciicode, SDL_keysym *keysym)
{
	...
	keysym->mod = KMOD_RESERVED;
	keysym->unicode = 0;
	// 041206: ghcstop unicode map delete 왜냐? 아예 keymap을 내맘대로 serial console용으로 변화시켰기 때문
